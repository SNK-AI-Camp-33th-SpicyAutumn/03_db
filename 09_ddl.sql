...
insert into product(name) values('텀플러');
...
DROP TABLE IF EXISTS user_unique;  # user_unique 삭제
CREATE TABLE IF NOT EXISTS user_unique (
    ...
    email VARCHAR(255)
    # , UNIQUE (phone)  # 테이블 레벨 제약 조건 설정 (가독성 우위)
) ENGINE=INNODB;
...
# [23000][1062] Duplicate entry '3-100-2026-06-12 11:31:33' for key 'order_composite_pk.PRIMARY'
...
#   : 부모 컬럼 값을 자식이 참조 중이면 변경, 삭제 시 자식 컬럼 값을 NULL로 변경
# - UPDATE/DELETE CASCADE
#   : 부모 컬럼 값을 자식이 참조 중이면 변경, 삭제 시 자식 테이블에서 참조 값이 포함된 모든 행을 삭제
...
# 컬럼 추가
alter table product
    add description varchar(255) not null default '설명 없음' after price;
