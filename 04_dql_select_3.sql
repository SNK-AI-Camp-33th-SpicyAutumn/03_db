...
### group by 사용 시 주의사항
# 1. null도 별도 그룹으로 묶임
# 2. select 절에는 group by 기준이 된 컬럼 + 그룹함수만 작성 가능
select
    ref_category_code
    # category_name  error 발생
from
    tbl_category
group by
    ref_category_code;
...
### where + group by: 필터링 된 행 중 컬럼 값이 같은 행을 그룹화
# - where: 지정된 테이블에서 행을 필터링
# - group by: 컬럼 값이 같은 행을 그룹화

# 메뉴 테이블에서 카테고리별 개수, 가격 합계를 구하기
...
join
    tbl_category cg
on
    m.category_code = cg.category_code
where
